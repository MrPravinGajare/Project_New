	...
	def __init__(self, *args, **kwargs):
		...
		def page_dnds():
			...
			try:
				...
				grid.addWidget(widgets["show_curve_radius_lds"][-1], 4, 2, 1, 2)
				grid.addWidget(widgets["show_curve_offset_lds"][-1], 4, 3, 2, 3)
				grid.addWidget(widgets["video_feed_lds"][-1], 5, 2, 1, 2)
		...
		def home_page():
			try:
				...
				header.setText("Road Side Safety Detection")
				...
				# Start Lane Detection System button
				...
				start_dnds = create_button("Auto Driving Helper System")
				...
				footer_text = \
					'Driving Helping System (DNDS) is a vehicle safety system that detects drivers ' \
				...
				grid.addWidget(widgets["start_lds"][-1], 3, 2, 1, 2)
